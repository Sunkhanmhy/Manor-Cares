...
| `FATAL: (ENOIDENTIFIER) no tenant identifier provided (external_id or sni_hostname required)` | The **pooler (Supavisor)** couldn't tell which Supabase project to route to. Every username on the pooler — including custom roles like `manor_app` from `create-app-role.sql` — **must** be suffixed with `.<project-ref>` (e.g. `manor_app.abcdefghijklmnop`), not just the bare role name. This is easy to miss when you create a custom role and hand-type the connection string instead of copying it. | Find your project ref (Supabase dashboard → Project Settings → General → "Reference ID", or the subdomain in your project's API URL `https://<project-ref>.supabase.co`), then edit `SUPABASE_DB_URL` so the user is `manor_app.<project-ref>` (or `postgres.<project-ref>` if connecting as the default role). |
